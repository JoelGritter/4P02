...
export const addAssignment = lambda(roleAuth(['admin', 'prof'], async (event, context, { userDoc }) => {
...
export const updateAssignment = lambda(roleAuth(['admin', 'prof'], async (event, context, { userDoc }) => {
	const resAssignment = await AssignmentModel.findById(event.pathParameters.id);
	const newAssignment = parseBody<Assignment>(event);

	const reqUser = userDoc as User;
	const { cognitoId } = reqUser;

	if(reqUser.roles.includes('admin') || resAssignment.createdBy == cognitoId){
		if(newAssignment._id) delete newAssignment._id;

		// prevent prof from removing themselves as assignment creator
		if(newAssignment.createdBy != cognitoId && newAssignment.createdBy != "admin"){
			newAssignment.createdBy = cognitoId;
		}

		const updatedAssignment = await AssignmentModel.findByIdAndUpdate(event.pathParameters.id, newAssignment, {
			new: true,
		});
...
export const deleteAssignment = lambda(roleAuth(['admin', 'prof'], async (event, context, { userDoc }) => {
	const resAssignment = await AssignmentModel.findById(event.pathParameters.id);
	...
	if(reqUser.roles.includes('admin') || resAssignment.createdBy == cognitoId){
		...
		const deleteRes = await AssignmentModel.findByIdAndDelete(event.pathParameters.id);
...
export const getAssignment = lambda(roleAuth(['admin', 'prof'], async (event, context, { userDoc }) => {
	const resAssignment = await AssignmentModel.findById(event.pathParameters.id);
