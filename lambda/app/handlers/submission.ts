...
export const getAssignmentSubmissions = lambda(
    auth(async (event, context, {userDoc}) => {
        ...
        else return unauthorized("Insufficient privileges: cannot retrieve submissions");
    })
);
...
export const grade = lambda(
    auth(async (event, context, {userDoc}) => {
        ...
        else return unauthorized('Cannot grade assignment: elevated course privileges required');
    })
);
...
export async function validateElevatedPrivileges(assignment: Assignment, reqUser: User) {
